...
export const Route = createFileRoute("/documento/advogado")({
  component: Page,
});
...
function Page() {
  ...
  const initialAdvNomeRef = useRef(doc.advNome ?? "");
  const [advOab, setAdvOab] = useState(doc.advOab ?? "");
  const [advEmail, setAdvEmail] = useState(doc.advEmail ?? "");
  const [advTelefone, setAdvTelefone] = useState(doc.advTelefone ?? "");
  ...
  function continuar() {
    ...
    setDoc({ advNome, advOab, advEmail, advTelefone });
    nav({ to: "/documento/ipl" });
  }

  return (
    <MobileShell title="Enviar documento" step={{ current: 2, total: 6 }} back="/documento/unidade">
      <PageTitle title="Seus dados" subtitle="Informe seus dados profissionais para contato e confirmação." />

      <div className="flex flex-col gap-4">
        <div>
          <label htmlFor="doc-adv-nome" className="mb-1.5 block text-sm font-medium text-foreground">
      ...
      <StepActions back="/documento/unidade" onNext={continuar} />
    </MobileShell>
  );
}
